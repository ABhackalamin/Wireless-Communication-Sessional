...
t = [1/Fs : 1/Fs : 1]; % total time for simulation = 1 second.
% Number of samples = 4000
Am = 1.0;
signal = Am * sin(2 * pi * 1000 * t); % Original signal

figure(1);
plot(t(1:200), signal(1:200), 'r') % red color
...
% Conversion of decimal into binary from least to most significant
indxtrn = index';
for i = 1:4000
    matrix(i, 1:8) = bitget(uint8(indxtrn(i)), 1:8);
end
% matrix is of 4000 rows X 8 columns
% matrixtps is a matrix of 8 rows X 4000 columns
matrixtps = matrix';
...
figure(2);
stairs(time(1:500), baseband(1:500), 'g') % green color
...
% Now, the binary converted data is sent to the Convolutional encoder.
...
% binary to decimal converted value
intconv = bi2de(convert); % converted into integer values (0-255) of 4000 samples
...
figure(3)
subplot(2, 1, 1)
plot(time(1:100), signal(1:100), 'b') % blue color
set(gca, 'ytick', [-1.0 0 1.0])
axis([0 time(100) -1 1])
...
subplot(2, 1, 2)
plot(time(1:100), sample_value(1:100), 'm') % magenta color
axis([0 time(100) -1 1])
set(gca, 'ytick', [-1.0 0 1.0])
